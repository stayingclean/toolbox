...
function tab(id){for(const b of document.querySelectorAll('[data-tab]'))b.setAttribute('aria-selected',String(b.dataset.tab===id));for(const p of document.querySelectorAll('[data-pane]'))p.hidden=p.dataset.pane!==id;}
for(const b of document.querySelectorAll('[data-tab]'))b.onclick=()=>tab(b.dataset.tab);
$('#toggle').onclick=()=>{const hidden=$('#panel').classList.toggle('collapsed');$('#toggle').setAttribute('aria-expanded',String(!hidden));};
try{
const components=new OBC.Components(),world=components.get(OBC.Worlds).create();
world.scene=new OBC.SimpleScene(components);world.scene.setup();world.scene.three.background=new THREE.Color('#edf0eb');
...
function planeRows(){const list=$('#plane-list');list.replaceChildren();for(const[id,p]of clipper.list){const row=document.createElement('div');row.className='measurement-row';const pick=document.createElement('button');pick.textContent=`Schnitt ${list.children.length+1}`;pick.onclick=()=>{activePlane=id;$('#section-slider').disabled=false;$('#section-slider').value=p.origin.dot(p.normal);};const del=document.createElement('button');del.textContent='×';del.title='Schnitt löschen';del.onclick=()=>run(async()=>{await clipper.delete(world,id);if(activePlane===id)activePlane=null;planeRows();});row.append(pick,del);list.append(row);}if(!clipper.list.size)list.textContent='Keine Schnitte.';$('#section-slider').disabled=!activePlane;}
function addPlane(normal,point){if(clipper.list.size>=12)throw Error('Maximal 12 Schnitte. Bitte zuerst einen löschen.');activePlane=clipper.createFromNormalAndCoplanarPoint(world,normal,point);clipper.visible=$('#section-helpers').checked;$('#section-slider').value=normal.dot(point);planeRows();return activePlane;}
for(const b of document.querySelectorAll('[data-cut-axis]'))b.onclick=()=>run(()=>addPlane(new THREE.Vector3(...JSON.parse(b.dataset.cutAxis)),center.clone()));
$('#section-slider').oninput=e=>run(async()=>{const p=clipper.list.get(activePlane);if(!p)return;const point=p.origin.clone().addScaledVector(p.normal,Number(e.target.value)-p.origin.dot(p.normal));p.setFromNormalAndCoplanarPoint(p.normal.clone(),point);await fragments.core.update(true);});
$('#section-flip').onclick=()=>run(()=>{const p=clipper.list.get(activePlane);if(p)p.setFromNormalAndCoplanarPoint(p.normal.clone().negate(),p.origin.clone());});$('#section-helpers').onchange=e=>clipper.visible=e.target.checked;$('#section-clear').onclick=()=>{clipper.deleteAll();activePlane=null;planeRows();};planeRows();
